...
document.getElementById("ai_n").innerText = "AI(" + difficulty + ")";
console.log("i work");
...
function saveGameHistory(scores){
    console.log("saving to database");

    var data = {
        _playerScore: scores['O'],
        _aiScore: scores['X'],
        _difficulty: difficulty
    }

    var xhr = new XMLHttpRequest();
    xhr.open("POST", "save_game_history.php", true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                console.log("Game History Saved successfuly!");
            }else{
                console.log("Error saving the game history: ", xhr.status);
            }
        }
    };
...
function toggleSettingsMenu() {
    ...
    var currentPosition = parseInt(getComputedStyle(settingsMenu).right);

    if (currentPosition === 0) {
        settingsMenu.style.left = '-300px'; // Hide the menu
    } else {
        settingsMenu.style.left = '0'; // Show the menu
    }
}
